...
int main() {
  ...
  {
    const Eigen::SparseMatrix<double> sparse = matrix_laplacian.transpose() * matrix_laplacian + penalty_coeff*penalty_coeff * matrix_penalty;
  ...
  while (!glfwWindowShouldClose(viewer.window)) {
    ...
    Eigen::MatrixXd rhs0 = matrix_laplacian.transpose() * matrix_laplacian * vtx_xyz_ref + penalty_coeff*penalty_coeff * matrix_penalty * vtx_xyz_def;
